...
const storageEvents = multer.diskStorage({
    destination: function (req, file, cb) {
        cb(null, './../FrontEnd/asset/img/events');
    },
...
const storagePosts = multer.diskStorage({
    destination: function (req, file, cb) {
        cb(null, './../FrontEnd/asset/img/posts');
    },
    filename: function (req, file, cb) {
        const name = file.originalname.toLowerCase().split(' ').join('-');
        const ext= MIME_TYPE_MAP[file.mimetype];
...
const storageTeam = multer.diskStorage({
    destination: function (req, file, cb) {
        cb(null, './../FrontEnd/asset/img/team');
    },
    filename: function (req, file, cb) {
        const name = file.originalname.toLowerCase().split(' ').join('-');
        const ext= MIME_TYPE_MAP[file.mimetype];
...
router.put('/:id/team',auth,multer({storage:storageTeam}).single("team_img") ,(req, res, next) => {
    ...
    club.updateOne({'team.team_id':req.body.team_id},{'$set':{'team.$':team}})
